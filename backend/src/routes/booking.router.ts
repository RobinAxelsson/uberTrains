...
router.post('/api/booking', async (req: Request, res: Response) => {
  try {
    ...
    let booking = await new BookingManager(new PaymentManager(), new mailService()).book(
      bookingDto,
    );
    res.json(booking);
  } catch (err) {
...
if (process.env.NODE_ENV === 'Development') {
  router.post('/api/fakebooking', async (req: Request, res: Response) => {
    try {
      ...
      let booking = await new BookingManager(new PaymentManagerStub(), new mailServiceStub()).book(
        bookingDto,
      );
      res.json(booking);
    } catch (err) {
...
router.post('/api/price', async (req: Request, res: Response) => {
  try {
    ...
    let bookingManager = new BookingManager(new PaymentManager(), new mailService());
